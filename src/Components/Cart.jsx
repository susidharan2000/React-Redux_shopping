...
import { ADD,REMOVE,SUB } from "../Utils/ProductSilce";

const Cart = () => {
  const list = useSelector((state)=>state.prodReducer.data);
    ...
    <>
      <div className="container h-100 d-flex align-items-center justify-content-center">
                  ...
                  <div className="left-container1 d-flex">
                    <div className="quantity-price">
                      <div className="price">
                        <div className="Qp">
                          <div>
                            <button className="add-btn" onClick={()=>dispatch(ADD(product.id))}>+</button>
                            <span className="quantity"> {product.quantity || 0} </span>
                            <button className="sub-btn" onClick={()=>dispatch(SUB(product.id))}>-</button>
                          </div>
                          <p>${(product.price * product.quantity).toFixed(2)}</p>{" "}
                          {/* Use product price */}
                        </div>
                      </div>
                      <button className="remove" onClick={()=>dispatch(REMOVE(product.id))}>Remove</button>
                    </div>
                  </div>
                  <div className="left-container2">
                    <button className="remove-btn btn btn-warning"onClick={()=>dispatch(REMOVE(product.id))}>
                      Remove
                    </button>
                    <div className="Qp">
                      <div>
        ...
        <div className="empty-Cart">
          <img src="https://external-content.duckduckgo.com/iu/?u=https%3A%2F%2Fwww.adasglobal.com%2Fimg%2Fempty-cart.png&f=1&nofb=1&ipt=5c82697bec9e3f5f4a516076618628b2c876070e943296f44dd2b8c0df14ba86&ipo=images" alt="" />
         </div>
         }
